...
public class ManagerSignup extends JFrame implements ActionListener {
	...
	ManagerSignup(String title) {
		...
		ImageIcon manager = new ImageIcon("images/manager.png");
		// imageicon 크기 조정을 위해 image 객체로 변경하여 크기 조정 후, 다시 imageicon화 함.
		Image managerImg = manager.getImage();
		Image imgManager = managerImg.getScaledInstance(100, 100, Image.SCALE_SMOOTH);
		ImageIcon managerIcon = new ImageIcon(imgManager);
		
		JRadioButton photoRB = new JRadioButton("관리자", managerIcon);
		main.add(photoRB);
		ct.add(main, FlowLayout.CENTER);
		...
		id = new JTextField("6자리 사번 (끝 2자리 부서번호)", 15);
		...
		main.add(p1); main.add(p2); main.add(p3); main.add(p4); main.add(p5); main.add(p6); main.add(p7);
		main.add(p8); main.add(p9); main.add(p10); main.add(p11); main.add(p12); main.add(p13); main.add(p14);
		...
		bottom.add(ok);
		bottom.add(cancel);
		ct.add(bottom, FlowLayout.CENTER);
	...
	public static void main(String[] args) {
		ManagerSignup win = new ManagerSignup("관리자 회원가입");
