...
class CalendarSwing extends JPanel implements ItemListener, ActionListener{
	Font fnt = new Font("",Font.PLAIN, 20);
	
	//상단
	JPanel selectPane = new JPanel(); //달력 년도 버튼 패널생성
...
//super : 부모클래스로부터 상속받은 메소드를 자식클래스에 참조해서 사용하는 변수
		...
		//상단
		selectPane.setBackground(new Color(153, 204, 255)); //백그라운드의 배경을 주입한다.
	...
	public void setDay() {
		//요일
		date.set(year, month-1, 1); //Calender 객체 date(현재날짜저장되어있음)를 세팅하는데, 일(day)를 1로 세팅한다.
